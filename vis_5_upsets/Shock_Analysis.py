...
app.layout = html.Div([
    ...
    html.Div([
        html.H4('An analysis of role different variables play on determining the unpredictability of match outcomes.'),
    ...
    html.Div([
        html.Div([
            html.H3('Shock vs Dropdown Value'),
            dcc.Graph(id='shock_graph'),
            ], className="six columns"),
        
        html.Div([
            html.H3('Player Rank by Dropdown Value'),
            dcc.Graph(id='dropdown_hist'),
            ], className="six columns"),
    ]),
    
    html.Div([
        html.H5('Highest Ranked Players and Corresponding Dropdown Value'),
        DataTable(id='rank_table', 
                #page_size=10,
                style_table={'maxWidth': '60%', 'height':'450px', 'overflowY':'auto'},
                style_cell={
                    # all three widths are needed
                    'minWidth': '40px', 'width': '100px', 'maxWidth': '100px',
                    'overflow': 'hidden',
    ...
    html.Div([
        html.H5('Biggest Upsets and Corresponding Dropdown Value'),
        DataTable(id='shock_table', page_size=10,
...
def rank_table(value):
    ...
    #dfc = dfc.iloc[:10,:]    # remove this and change it to a scrollable datatable
    #(styles, legend) = discrete_background_color_bins(dfc)
...
#    dfd['rank_diff'] = dfd.winner_rank - dfd.loser_rank
    dfd = dfd.sort_values(by=['shock_factor'], ascending=False)
    dfd = dfd[['winner_name','loser_name','winner_rank', 'loser_rank',newcols[0], newcols[1], 'tourney_name']]
    #dfd = dfd.iloc[:10,:]    # remove this and change it to a scrollable datatable
